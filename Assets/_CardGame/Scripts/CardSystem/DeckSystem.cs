...
namespace _CardGame.Scripts.CardSystem
{
    [Serializable]
    public class DeckSystem<T>
    {
        ...
        public void Draw(int numberToDraw = 1)
        {
            ...
            if (DrawPile.Count >= numberToDraw) {
                var drawnCards = DrawPile.Draw(numberToDraw);
                Hand.Add(drawnCards);
            }
            // not enough cards! Trigger overdraw event
                ...
                // draw the cards
                var drawnCards = DrawPile.Draw(numberToDraw);
                // send out notification on remaining cards
        ...
        public void DrawFromDiscard(int numberToDraw = 1)
        {
            ...
            var drawnCards = DiscardPile.Draw(numberToDraw, false);
        ...
        {
            if (cardIndex < 0 || cardIndex > Hand.Cards.Count - 1) {
                return;
            } // index doesn't fall within range
        ...
        {
            if (cardIndexes.Count > Hand.Cards.Count) {
                return;
            } // can't discard more than we have

            // create the list of cards to remove
            var cardsToDiscard = new List<T>();
